...
function evaluateAuthorizationHeader(req, res, next) {
  ...
  if (!header) {
    ...
    return res.status(HTTP.UNAUTHORIZED).json('No authorization header found');
  }
  ...
  if (auth.length == 1) {
    ...
    return res.status(HTTP.UNAUTHORIZED).json('Invalid token header. No credentials provided.');
  }
  if (auth.length > 2) {
    // Invalid token header (spaces in token); respond with 401
    //console.log('**** invalid header (spaces in token)');
    return res.status(HTTP.UNAUTHORIZED).json('Invalid token header. Token string should not contain spaces.');
  }
...
function checkValidImage(req, res, next) {
  //console.log('**** checking for valid image');
  if (!(req.files && req.files.image)) {
    return res.status(HTTP.BAD_REQUEST).json('No image sent');
  }
  return next();
}
...
module.exports = {
  authenticate,
  checkDivesiteOwnership,
  checkValidImage,
  evaluateAuthorizationHeader,
};
